...
function FeatureCard({ title, description, href, cta = 'Open', highlight, icon: Icon }: FeatureCardProps) {
    ...
    const cardSx = {
        ...
        '&:hover': { boxShadow: highlight ? 6 : 4, transform: 'translateY(-2px)', borderColor: highlight ? theme.palette.secondary.main : theme.palette.primary.main },
        ...
        <Card elevation={highlight ? 4 : 1} tabIndex={0} sx={cardSx}>
            <CardContent sx={{ flexGrow: 1 }}>
                <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                    {Icon && (
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: '50%', background: iconBg, color: highlight ? theme.palette.secondary.main : theme.palette.primary.main }}>
                            <Icon fontSize="small" />
                        </Box>
                    )}
